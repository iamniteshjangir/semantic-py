...
from pysemantic.validation.common.validation_constants import ALL_RESERVED_WORDS, VALID_IDENTIFIER_PATTERN
...
class ModelValidation:
    ...
    def _validate_model_name_is_valid_identifier(self) -> None:
        """Rule 2: Model names must be valid identifiers."""
        if not self._is_valid_identifier(self.model.name):
            if self.model.name in ALL_RESERVED_WORDS:
                message = (
                    "Model name cannot be a reserved word or SQL keyword. "
    ...
    def _validate_table_name_is_valid_identifier(self) -> None:
        """Rule 3: Table names must be valid identifiers."""
        if not self._is_valid_identifier(self.model.table):
            if self.model.table in ALL_RESERVED_WORDS:
                message = (
                    "Table name cannot be a reserved word or SQL keyword. "
