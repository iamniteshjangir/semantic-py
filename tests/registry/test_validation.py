...
from pysemantic.modeling import Model, Entity, Dimension, Measure, EntityType
...
def create_model(name, table="table_1", grain="daily", measures=None, dimensions=None, entities=None, time_columns=None):
    if measures is None:
        measures = []
...
def test_valid_registry():
    ...
    validator = RegistryValidation(
        models={"users": users, "orders": orders},
        entities={},
        dimensions={},
        measures={}
    )
...
def test_identical_models_subtle_difference():
    """Test that subtly different models on the same table are ALLOWED (not identical)."""
    model1 = Model(
        name="model1",
        table="common_table",
        primary_key="id",
